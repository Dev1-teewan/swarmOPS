    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-zinc-900 border-[#ddf813]/50 text-white backdrop-blur-lg max-w-4xl">
        <DialogHeader className="flex flex-row items-center justify-between">
          <DialogTitle className="text-2xl font-semibold text-[#ddf813">
            Welcome To SwarmOPS
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="mt-3 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {features.map((feature) => {
              ...
              return (
                <div
                  key={feature.title}
                  className="space-y-2 p-4 rounded-lg border border-[#ddf813]/20 bg-zinc-800"
                >
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-[#ddf813]">
              Getting Started
            </h3>
            <div className="space-y-3">
              {steps.map((step, index) => (
                ...
                >
                  <ArrowRight className="w-4 h-4 text-[#ddf813]" />
                  <span>{step}</span>
                </div>
              ))}
            </div>
