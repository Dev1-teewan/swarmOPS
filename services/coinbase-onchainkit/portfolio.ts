...
import { getPortfolios, GetPortfoliosResponse, APIError } from '@coinbase/onchainkit/api';
...
export const getCombinedPortfolio = async (addresses: `0x${string}`[]): Promise<ICombinedPortfolio> => {
  setOnchainKitConfig();
  ...
  return combinePortfolios((portfolioResponse as GetPortfoliosResponse).portfolios) ;
}
...
export const combinePortfolios = (portfolios: CoinbasePortfolio[]): ICombinedPortfolio => {
  ...
  portfolios.forEach(p => {
    totalPortfolioBalanceInUsd += parseFloat(p.portfolioBalanceInUsd.toString());
